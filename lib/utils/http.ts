...
export async function getBalanceMetadata(
    endpoint: string,
    denom: string
): Promise<{ metadata: CoinMetadata }> {
    const url = `${endpoint}/cosmos/bank/v1beta1/denoms_metadata/${denom}`;
    return get(url);
}

export async function getIBCDenomMetadata(
    denom: string
...
export const getAuthority = async (endpoint: string) => {
    return get(`${endpoint}/cosmos/upgrade/v1beta1/authority`);
};
